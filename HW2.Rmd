...
This cluster represents the lowest-impact players, characterized by extremely limited playing time and minimal statistical production across all categories. The cluster is dominated by centers ($36.4\%$) and forwards ($45.5\%$), with only 2 guards ($18.2\%$), reflecting an interior-oriented composition. While they show relatively high two-point shooting efficiency ($79.9\%$), this reflects shot selection rather than offensive skill, as evidenced by their very poor three-point shooting and free throw shooting. Their contributions to rebounds, assists, steals, and blocks are all minimal, resulting in the lowest efficiency rating and scoring output. These are, indeed, deep bench reserves, who see action only in limited situations to provide size or rest for starters.

\textbf{Cluster 2: Perimeter Role Players}

This cluster captures perimeter-oriented players with moderate playing time who specialize in outside shooting. The cluster is heavily dominated by guards ($50.0\%$) and forwards ($38.9\%$), reflecting a clear perimeter orientation. Their profile shows below-average overall field goal percentage and poor two-point shooting, but respectable three-point shooting, indicating they operate primarily away from the basket. Their free throw shooting is moderate, and they contribute modestly to assists and steals, but provide minimal interior presence with very low blocks and rebounds. With $4.99$ points and $5.69$ efficiency, they represent bench-level perimeter specialists. In other words, primarily guards who enter games to provide outside shooting but lack the overall impact or minutes of rotation starters.

\textbf{Cluster 3: Balanced starters with strong contribution}

This cluster represents high-quality rotation players who receive substantial playing time and contribute across multiple dimensions. The cluster shows a pretty balanced positional composition with guards ($52.9\%$), forwards ($41.2\%$) and centers ($5.9\%$), suggesting these are versatile players regardless of nominal position. They show balanced shooting profiles with moderate overall field goal percentage, acceptable two-point shooting, and strong three-point shooting, indicating versatility in shot selection. Their excellent free throw shooting reflects technical skill, and they contribute significantly to playmaking, rebounding, and defensive activity. With $11.3$ points and $13.3$ efficiency, these are the core rotation players: reliable, versatile contributors who can impact the game in multiple ways and handle significant minutes, though they are not the primary offensive options.

\textbf{Cluster 4: Elite Performers}

This cluster contains the highest-impact players, characterized by heavy playing time and dominance across virtually all statistical categories. Interestingly, this elite cluster contains only forwards, with no guards or centers. They excel in scoring, efficiency, and rebounding ($9.60$ total rebounds, with exceptional offensive rebounding at $3.20$), while also contributing significantly to playmaking ($4.30$ assists), steals, and blocks. Their shooting profile shows strong two-point percentage and good free throw shooting, though their three-point percentage is lower, likely because they operate in diverse scoring areas rather than specializing from distance. The elevated turnover rate reflects their high usage and ball-handling responsibilities rather than poor decision-making. These are elite players who carry their teams' offensive and defensive responsibilities, dominating statistical production across the board.

######## laia

\textbf{- Cluster 1: Low-impact perimeter players}
This group gathers players with moderate playing time ($\approx 15$ minutes) and overall modest statistical contribution. Their scoring efficiency is average across FG, 2PT and 3PT percentages, and they provide limited rebounding ($\approx 2$ REB) and moderate assists ($\approx 1.5$ AST).
...
Players in this cluster show the best shooting percentages in the dataset, especially 2PT FG ($\approx 80%$), but they play the fewest minutes ($\approx 10$).
Their statistical output is low in totals due to reduced court time, but their efficiency is notably high relative to minutes ($EFF \approx 4.8$).
...
This small but meaningful cluster contains players with very high minutes ($\approx 30$) and elite production across all categories: $REB \approx 9.6$, $AST \approx 4.3$, $EFF \approx 28$ and $PTS \approx 24$.
...
Players in this cluster have high playing time ($\approx 24$ minutes) and solid all-around contributions: 3PT FG highest among clusters ($\approx 43\%$), $AST \approx 3$, $PTS,\approx 11$ and $EFF \approx 13$.
...
Therefore, this cluster represents \textbf{standard rotation} players who play above-average minutes and \textbf{specialize in three-point shooting}. In other words, primarily guards who contribute through perimeter shooting and ball movement rather than all-around production. They represent the middle class of professional basketball.
...
It is worth noting that minutes played is no longer a dominant discriminating variable, as it was in the PCA-based clustering. Compared to the PCA results, this indicates that the MCA-based approach captures a broader range of performance characteristics, rather than focusing primarily on the most extreme cases.
...
The confusion matrix, where rows correspond to the true classes and columns to the predicted classes, shows that the model performs best when classifying Guards, with 18 correctly classified observations out of 20. The second best classified group is Forwards, with 15 correct predictions out of 22. In contrast, the model performs worst for Centers, correctly identifying only 4 out of 7 observations, with most misclassifications occurring as Forwards.

The overall Correct Classification Rate (CCR) is 75.5%. The class-specific CCRs highlight the unequal performance across groups: Centers are the least accurately predicted class with a CCR of 57.14%, followed by Forwards with 68.18%, while Guards achieve the highest CCR at 90%.

For comparison, the baseline accuracy, computed from the class proportions, is 38.86%. This value represents the expected accuracy under random classification while preserving the observed class distribution. The model’s CCR therefore represents a substantial improvement over the baseline accuracy.
...
In our case, the test shows a p-value of 0.48, we fail to reject the null hypothesis. This result indicates that, despite achieving a higher classification accuracy than the baseline, the observed improvement is not statistically significant when compared to random classification under equal class priors. 

We also compute the critical value of the chi-squared distribution at the 5% significance level with $k - 1$ degrees of freedom. And since the observed Q statistic does not exceed this critical value, the test statistic does not fall within the rejection region, further confirming that the null hypothesis cannot be rejected.
