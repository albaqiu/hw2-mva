...
FG does not pass the Shapiro-Wilk normality test, since its p-value is 0.01795. However, using the Kolmogorov test, the normality is not rejected, this, adding the fact that its histogram is approximately bell-shaped, we will consider it to be approximately normal for this analysis.
```{r}
...
The contingency table (confusion matrix), where rows represent the true classes and columns the predicted classes, shows that the model performs better when classifying Guards, they have 18 correctly classified observations out of 20. Then, the second best classified group is Forward, with 15 out of 22 correct classifications. In contrast, the model struggles with Centers, identifying correctly only 4 out of 7 and misclassifying most of them as Forwards.

The overall Correct Classification Rate (CCR) is 75.5%, and the class-specific CCRs confirm the unequal performance across groups: Centers are the worst predicted class (57.14%), then, the Forwards achieve a CCR of 68.18% and lastly, Guards achieve a CCR of 90%.
